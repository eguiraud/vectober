// This is basically llvm::SmallVector
...
// RVec is a SmallVector
template <typename T>
using RVec = SmallVector<T, 8>;
...
template <typename T>
inline void swap(ROOT::SmallVectorImpl<T> &LHS, ROOT::SmallVectorImpl<T> &RHS)
{
   LHS.swap(RHS);
}
...
/// Implement std::swap in terms of SmallVector swap.
template <typename T, unsigned N>
inline void swap(ROOT::SmallVector<T, N> &LHS, ROOT::SmallVector<T, N> &RHS)
{
   LHS.swap(RHS);
}
