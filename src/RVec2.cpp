...
{
   std::string Reason = "SmallVector capacity unable to grow. Already at maximum size " + std::to_string(SizeTypeMax());
   throw std::length_error(Reason);
}
...
template class ROOT::SmallVectorBase<uint32_t>;
...
template class ROOT::SmallVectorBase<uint64_t>;
